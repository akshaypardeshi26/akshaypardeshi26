...
void randomForest()
{
  ...
  /* printf("Data Copied in to 2D array:\n");
    ...
    printf("\nTrain Data:\n");
    printf("Traning data validation is just to check overfitting.Can be ignored\n");
    ...
    for(int i=0;i<trainSize;i++)
    {
        if(testInput(t_node, indata[i])==indata[i][attributes])
           cnt++;
            ...
            printf("missClassified: Sample Num:%d ,output = %d expected = %d\n",i,testInput(t_node, indata[i]),indata[i][attributes]);
    }
